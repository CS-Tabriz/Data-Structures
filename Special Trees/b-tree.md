...
4. **پیمایش مرتب (In-order Traversal):** متد traverse به صورت بازگشتی تمام نودها را ملاقات می‌کند. عملکرد آن به این صورت است که ابتدا فرزند سمت چپ یک کلید را بررسی می‌کند، سپس خود کلید را چاپ کرده و بعد به سراغ فرزند سمت راست می‌رود. این الگوریتم تضمین می‌کند که مهم نیست داده‌ها با چه ترتیبی وارد شده‌اند، خروجی نهایی همیشه یک لیست کاملا مرتب‌شده از کوچک به بزرگ خواهد بود.


### 5️⃣ نکات تکمیلی و جمع‌بندی

درخت B یک ساختار داده‌ی چندشاخه‌ای، متوازن و مناسب برای حافظه‌های ثانویه است که برای مدیریت حجم زیاد داده طراحی شده است. ایده‌ی اصلی آن این است که:

- ارتفاع درخت کوچک بماند.
- هر نود تا حد ممکن پر باشد.
- تعداد Disk I/O حداقل شود.

به همین دلیل، B-Tree یکی از پایه‌های اصلی در طراحی Database Management Systems (DBMS) و File Systems محسوب می‌شود.


#### رابطه‌ی درجه و ارتفاع

در یک B-Tree اگر درجه‌ی حداقل را با $t$ نمایش دهیم، ساختار درخت مستقیما به این پارامتر وابسته است. هر نود غیرریشه باید حداقل $t - 1$ کلید داشته باشد و در مقابل، هیچ نودی نمی‌تواند بیش از $2t - 1$ کلید نگه دارد. از آنجا که تعداد فرزندان هر نود یک واحد بیشتر از تعداد کلیدهای آن است، حداکثر تعداد فرزندان برابر با $2t$ خواهد بود. این محدودیت‌ها تضمین می‌کنند که درخت نه بیش از حد خلوت شود و نه بیش از حد متراکم، و همواره در حالت متوازن باقی بماند.

$$
h \le \log_t \frac{n+1}{2}
$$

مشاهده می‌کنیم که ارتفاع درخت به صورت لگاریتمی نسبت به تعداد کلیدها رشد می‌کند. نکته‌ی مهم این است که پایه‌ی لگاریتم برابر با $t$ است؛ بنابراین هرچه مقدار $t$ بزرگ‌تر انتخاب شود، مقدار $h$ کوچک‌تر خواهد بود. این ویژگی در کاربردهای مبتنی بر دیسک اهمیت ویژه‌ای دارد، زیرا کاهش ارتفاع به معنای کاهش تعداد دسترسی به بلوک‌های حافظه‌ی ثانویه است. به همین دلیل در سیستم‌های پایگاه داده معمولا مقدار $t$ را نسبتاً بزرگ در نظر می‌گیرند، به‌گونه‌ای که حتی برای میلیون‌ها رکورد نیز ارتفاع درخت اغلب از سه یا چهار سطح فراتر نمی‌رود.


#### بدترین و بهترین حالت

در بهترین حالت، تمام نودهای درخت کاملا پر هستند؛ یعنی هر نود شامل $2t - 1$ کلید است. در این وضعیت تعداد کلیدها در هر سطح حداکثر بوده و در نتیجه ارتفاع درخت به کمترین مقدار ممکن می‌رسد. به بیان دیگر، درخت بیشترین تراکم ساختاری را دارد و با کمترین تعداد سطح، بیشترین داده را در خود جای می‌دهد.

در مقابل، در بدترین حالت هر نود (به‌جز ریشه) تنها دارای حداقل تعداد مجاز یعنی $t - 1$ کلید است. در این شرایط درخت از نظر ساختاری کم‌تراکم‌ترین حالت ممکن را دارد و برای نگهداری همان تعداد کلید، به سطوح بیشتری نیاز خواهد داشت؛ بنابراین ارتفاع به بیشترین مقدار خود می‌رسد.

با این حال، در هر دو وضعیت، رشد ارتفاع همچنان از مرتبه‌ی لگاریتمی نسبت به تعداد کلیدها باقی می‌ماند و پیچیدگی زمانی عملیات‌ها از چارچوب $O(t.\log n)$ خارج نمی‌شود.


### 6️⃣ سوالات تمرینی

1. در یک B-Tree با درجه‌ی حداقل $t = 3$، حداکثر تعداد کلید در هر نود چند است؟
2. در یک B-Tree با درجه‌ی حداقل $t = 3$، حداکثر تعداد کلید در هر نود چند است؟
3. در B-Tree همه‌ی برگ‌ها در چه وضعیتی هستند؟
4. در حذف، اگر یک نود کمتر از $t-1$ کلید داشته باشد، ابتدا چه کاری انجام می‌شود؟
5. یک B-Tree با $t = 2$ داریم. عناصر زیر را به ترتیب درج کنید:
```
[15, 8, 25, 3, 12, 20, 30]
```
ساختار نهایی درخت را رسم کنید.

6. اگر یک B-Tree با درجه $t = 4$ داشته باشیم:
    - **الف:** حداقل تعداد کلید در یک نود غیرریشه چند است؟
    - **ب:** حداکثر تعداد کلید چند است؟
    - **ت:** حداکثر تعداد فرزند چند است؟
