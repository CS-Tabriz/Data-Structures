...
**الگوریتم:** مجموعه‌ای از دستورالعمل‌های مرحله‌به‌مرحله و مشخص، است که برای حل یک مسئله یا انجام یک کار طراحی شده و در نهایت به یک پاسخ یا نتیجه می‌رسد. 

بنابراین معمولا هر برنامه کامپیوتری شامل یک یا چند الگوریتم است که دارای ورودی، خروجی، مراحل مشخص، محدود و قابل اجرا می‌باشد.  

 حال که می‌دانیم الگوریتم چیست، سوال مهم این است که وقتی برای یک مسئله چندین الگوریتم مختلف وجود دارد، چگونه تشخیص دهیم کدام بهتر (کارآمدتر) است؟ 
ما در تحلیل الگوریتم‌ها بیشتر بر روی کارایی(زمان اجرا) تمرکز می‌کنیم. 
و پیش از آن بهتر است نیم‌نگاهی بر اصول ریاضیات داشته باشیم.
> ## مقدمات ریاضی
>> ### * خواص سیگما :
...
**زمان اجرا:**
کارایی الگوریتم به این معنی است که هر الگوریتمی که با مصرف کمتر منابع (زمان و حافظه)، مسئله را حل کند، بهتر است.
اما مشکلی که وجود دارد این است که اندازه‌گیری زمان یا مقدار حافظه دقیق همیشه ثابت نیست و به مواردی مثل سخت‌افزار، زبان برنامه‌نویسی، یا داده‌های ورودی وابسته است.
بنابراین نیازمند روشی هستیم که مستقل از این عوامل متغیر باشد.

  به روشی نیازمندیم که رشد تعداد عملیات یک الگوریتم را نسبت به اندازه‌ی ورودی‌اش (n) بیان کند، بدون این‌که وارد جزئیات سخت‌افزار یا زمان واقعی اجرا شود.


# رشد توابع
به رفتار تابعی مثل تعداد عملیات یا مصرف حافظه وقتی اندازه‌ی ورودی به سمت بی‌نهایت (∞ → n ) می‌رود، اشاره دارد. که بیشتر به سرعت افزایش تابع در مقیاس بزرگ مدنظر است، نه مقدار دقیق آن.

  به‌طور ساده‌تر رشد تابع یعنی بررسی اینکه یک تابع وقتی مقدار ورودی‌اش (n) بزرگ‌تر می‌شود، چقدر سریع افزایش پیدا می‌کند.

به طور کلی حد $\lim_{n \to \infty} \frac{f(n)}{g(n)}$ وقتی $n \to \infty$ سه حالت دارد:

* اگر جواب صفر باشد، رشد g بیشتر است.
* اگر جواب $\infty$ باشد، رشد f بیشتر است.
* اگر جواب عدد ثابتی باشد، هر دو هم‌رشد هستد.
...
به مبهم رسیدیم و با هوپیتال پیش می‌رویم:
...
تعداد اجراها به صورت سری هندسی $\frac{n}{2} + \frac{n}{4} + \frac{n}{8} + \dots$ خواهد بود که مجموع آن همگرا به $n$ است.
**نتیجه:** $\theta(n)$.
```c
for(i = 1; i <= n; i++){
